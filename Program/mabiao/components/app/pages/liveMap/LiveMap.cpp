#include "PageManager.h"  // 必须在LiveMap.h之前包含，以便PageManager是完整类型
#include "LiveMap.h"
#include "config.h"
#include <cstdio>
#include <string.h>
#include <inttypes.h>  // 用于PRId32宏
...
void LiveMap::onViewWillAppear()
{
    ...
    Model.SetStatusBarStyle(STATUS_BAR_STYLE_BLACK);
    SportInfoUpdate();
...
{
    /* 设置 lastMapUpdateTime 使得下次 Update() 刚好在 ms 毫秒后触发刷新
     * Update() 的触发条件是 elapsed >= CONFIG_GPS_REFR_PERIOD
     * 因此需要令 (now - lastMapUpdateTime) + ms == CONFIG_GPS_REFR_PERIOD
     */
    priv.lastMapUpdateTime = lv_tick_get() + ms - CONFIG_GPS_REFR_PERIOD;
}
...
void LiveMap::SportInfoUpdate()
{
    lv_label_set_text_fmt(
        ...
        "%02d",
        (int)Model.sportStatusInfo.speed_kph);

    lv_label_set_text_fmt(
        View.ui.sportInfo.labelTrip,
        "%0.1f km",
        Model.sportStatusInfo.single_distance / 1000);

    char buf[16];
    lv_label_set_text(
        View.ui.sportInfo.labelTime,
        LiveMapModel::MakeTimeString(Model.sportStatusInfo.single_time, buf, sizeof(buf)));
}
...
void LiveMap::onEvent(lv_event_t *event)
{
    ...
    if (code == LV_EVENT_GESTURE)
    {
        ...
        int32_t curLevel = lv_slider_get_value(instance->View.ui.zoom.slider);
        int32_t newLevel = (dir == LV_DIR_LEFT) ? (curLevel - 1) : (curLevel + 1);
        int32_t levelMin = instance->Model.mapConv.GetLevelMin();
        int32_t levelMax = instance->Model.mapConv.GetLevelMax();

        /* 边界保护：超出范围不再触发刷新，避免无效重载 */
        if (newLevel < levelMin || newLevel > levelMax || newLevel == curLevel)
        {
            return;
        }

        lv_slider_set_value(instance->View.ui.zoom.slider, newLevel, LV_ANIM_OFF);
        lv_label_set_text_fmt(instance->View.ui.zoom.labelInfo, "%" PRId32 "/%" PRId32,
                              newLevel, levelMax);

        lv_obj_clear_state(instance->View.ui.zoom.cont, LV_STATE_USER_1);
        instance->priv.lastContShowTime = lv_tick_get();

        /* 缩放希望立即响应，让下一次 Update() tick (100ms 内) 就刷新地图 */
        instance->UpdateDelay(0);

#if CONFIG_LIVE_MAP_DEBUG_ENABLE
        printf("[TOUCH] %s:%" PRId32 "\n",
               (dir == LV_DIR_LEFT) ? "LEFT" : "RIGHT", newLevel);
#endif
    }
