...
{
    lv_obj_set_style_bg_color(root, lv_color_white(), 0);  // 再次反色：黑色改回白色

    lv_obj_t* label = lv_label_create(root);
    lv_obj_center(label);
    lv_obj_set_style_text_font(label, ResourcePool::GetFont("bahnschrift_17"), 0);
    // 移除文字颜色设置，使用默认颜色（在白色背景上通常是黑色）
...
void LiveMapView::SportInfo_Create(lv_obj_t* par)
{
    ...
    lv_obj_set_style_text_color(label, lv_color_white(), 0);  // 再次反色：黑色改回白色
